...
async def main():  # 1
    start = time()
    coffee = asyncio.create_task(make_coffee(start))
    egg = asyncio.create_task(fry_eggs(start))
    await coffee  # run task with await
    await egg
    print(f"breakfast is ready in {time()-start:.2f} secs.")
